...
# Count the frequency of each team in the Event column
event_counts <- table(data$Event)
top_50_events <- names(sort(event_counts, decreasing = TRUE))[1:50]
data$Event <- ifelse(data$Event %in% top_50_events, as.character(data$Event), "Other")
table(data$Event)
...
rf_model_Before_1985 <- randomForest(MedalBinary ~ ., 
                         data = train_data, 
                         ntree = 1000,      
                         mtry = default_mtry, 
                         importance = TRUE)

print(rf_model_Before_1985)


importance(rf_model_Before_1985)
varImpPlot(rf_model_Before_1985)

######## Predict on the test dataset
predictions <- predict(rf_model_Before_1985, newdata = test_data)
predicted_probabilities <- predict(rf_model_Before_1985, newdata = test_data, type = "prob")[, 2]
